...
If the memory area reserved for the list is already full and there is no room for the new element, more work is required. In such a case, a new bigger memory area is reserved for the list and all elements are moved to the new area, which needs $$O(n)$$ time. However, with a [suitable implementation](../list-addition) such a situation can be made so rare that the <i>amortized</i> time complexity of the `append` method is $$O(1)$$. Here amortized $$O(1)$$ time means that any series of $n$ operations takes $O(n)$ time.
...
As with adding, removing an element at the end of a list takes amortized $$O(1)$$ time when using [suitable implementation](../list-addition), because the other elements are not affected. After removing the element `6`, the memory might look like this:
...
*) amortized complexity, see [suitable implementation](../list-addition).
...
In addition to array-like implementation of lists, there are also linked lists. <i>Single-linked list</i> can be implemented by defining a class with value and reference fields. The reference points to the (next) instance of the same class. Keeping track of the head (start) and tail (end) of such linked list enables constant time append(), prepend() (add to the beginning) and pop(0). Adding reference to the previous instance gives so-called <i>doubly-linked list</i>, that enables to implement pop() in constant time. Accessing the i-th item takes $$O(i)$$ time as one needs to scan from the start through the references. A benefit to array-like lists is that now one can e.g. delete an item from the middle in constant time: if you have a reference to an item v, you can just follow its references to previous and next items and modify their references to skip v. For more details, see 
[doubly-linked list](../linked-list)

Linked lists give thus efficient way to implement <i>stack</i> and <i>queue</i>, the former supporting append() and pop(), and the latter supporting append(), pop(0). With such implementation, both support also accessing the first and last elements in constant time.

An example implementation of single-linked list is given below.
